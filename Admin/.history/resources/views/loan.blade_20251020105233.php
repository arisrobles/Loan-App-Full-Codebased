      ...
      <table class="w-full text-sm">
        ...
        <tbody class="divide-y divide-slate-100">
          @foreach($loans as $row)
            @php
              $statusKey = $row->status ?? 'new_application';
            @endphp
            <tr class="hover:bg-slate-50">
              ...
              <td class="p-3"><span class="badge bg-slate-100 text-slate-700">{{ ucfirst(str_replace('_',' ',$statusKey)) }}</span></td>
              <td class="p-3">₱{{ number_format(max(0, (float)$row->total_disbursed - (float)$row->total_paid),2) }}</td>
              <td class="p-3">
                <details class="relative">
                  <summary class="cursor-pointer px-3 h-9 rounded border border-slate-200 hover:bg-slate-50 inline-flex items-center">Actions ▾</summary>
                  <div class="absolute z-20 mt-1 w-56 bg-white border border-slate-200 rounded-lg shadow-card p-2 space-y-1">
                    ...
                    @if($statusKey !== 'approved')
                      <form method="POST" action="{{ route('loans.move',['loan'=>$row->id]) }}">
                        @csrf
                        <input type="hidden" name="to_status" value="{{ \App\Models\Loan::ST_APPROVED }}">
                    ...
                    @if($statusKey === 'approved')
                      <form method="POST" action="{{ route('loans.move',['loan'=>$row->id]) }}">
                        @csrf
                        <input type="hidden" name="to_status" value="{{ \App\Models\Loan::ST_FOR_RELEASE }}">
                        <button class="w-full text-left px-3 py-2 rounded hover:bg-slate-50" type="submit">Mark For Release</button>
                      </form>
                    @endif

                    {{-- DISBURSE --}}
                    @if(in_array($statusKey,[\App\Models\Loan::ST_FOR_RELEASE,\App\Models\Loan::ST_APPROVED]))
                      <form method="POST" action="{{ route('loans.move',['loan'=>$row->id]) }}">
                        @csrf
                        <input type="hidden" name="to_status" value="{{ \App\Models\Loan::ST_DISBURSED }}">
                        <div class="px-3 py-2">
                          <label class="text-xs text-slate-600">Release date</label>
                          <input type="date" name="release_date" class="mt-1 w-full border rounded px-2 h-8 text-sm">
                        </div>
...
@push('scripts-head')
<script>
  window.labels   = @json($chartLabels);
  window.datasets = @json($chartDatasets);
</script>
@endpush
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  const datasets=keys.map((label,i)=>({
    label,data:datasetsMap[label],
    backgroundColor:palette[i%palette.length],
    borderColor:palette[i%palette.length],
    borderWidth:1,borderRadius:8,maxBarThickness:36,stack:'pipeline'
  }));
  ...
  new Chart(ctx,{
    ...
    options:{
      ...
      plugins:{
        ...
        tooltip:{backgroundColor:'#0f172a',titleColor:'#fff',bodyColor:'#e5e7eb',padding:10,callbacks:{label:ctx=>` ${ctx.dataset.label}: ${ctx.parsed.y}`}}
      },
      scales:{x:{stacked:true,grid:{display:false}},y:{stacked:true,grid:{color:'rgba(148,163,184,.2)'},ticks:{precision:0,stepSize:1}}}
    }
  });
});
</script>
