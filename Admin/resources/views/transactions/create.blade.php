...
@section('content')
    ...
    <div class="px-6 py-5 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
      <div>
        ...
        <p class="text-sm text-slate-200">
          Manually record a bank or journal entry for this account.
        </p>
      </div>
  ...
  <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
    <form action="{{ route('transactions.store', $account->id) }}" method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-4">
      @csrf
      ...
      <div>
        <label class="block text-xs font-semibold text-slate-600 mb-1">Kind <span class="text-rose-500">*</span></label>
        <select name="kind" class="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm" required>
      ...
      <div>
        <label class="block text-xs font-semibold text-slate-600 mb-1">Transaction Date <span class="text-rose-500">*</span></label>
        <input type="date" name="tx_date" value="{{ old('tx_date', now('Asia/Manila')->toDateString()) }}"
      ...
      <div>
        <label class="block text-xs font-semibold text-slate-600 mb-1">Spent (Money Out)</label>
        <input type="number" step="0.01" min="0" name="spent" value="{{ old('spent') }}"
               class="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm">
      </div>

      <div>
        <label class="block text-xs font-semibold text-slate-600 mb-1">Received (Money In)</label>
        <input type="number" step="0.01" min="0" name="received" value="{{ old('received') }}"
               class="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm">
      </div>
      ...
      <div>
        ...
        <select name="reconcile_status" class="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm">
          ...
          <option value="pending" @selected(old('reconcile_status')==='pending')>Pending</option>
          <option value="ok"      @selected(old('reconcile_status')==='ok')>OK</option>
          <option value="match"   @selected(old('reconcile_status')==='match')>Matched</option>
        </select>
      </div>
      ...
      <div>
        ...
        <select name="status" class="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm">
          ...
          <option value="excluded" @selected(old('status')==='excluded')>Excluded</option>
        </select>
      </div>

      <div>
        <label class="block text-xs font-semibold text-slate-600 mb-1">Ledger Contact</label>
        <input type="text" name="ledger_contact" value="{{ old('ledger_contact') }}"
               class="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm">
      </div>

      <div>
        <label class="block text-xs font-semibold text-slate-600 mb-1">Account Name (Chart of Accounts)</label>
      ...
      <div>
        ...
        <input type="text" name="tx_class" value="{{ old('tx_class') }}"
               class="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm">
      </div>
      ...
      <div>
        ...
        <input type="text" name="source" value="{{ old('source') }}"
               class="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm" placeholder="Manual, CSV import, etc.">
      </div>
