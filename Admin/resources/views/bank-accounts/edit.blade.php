...
@section('content')
  ...
  <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 max-w-xl">
    <form action="{{ route('bank-accounts.update', $account) }}" method="POST" class="space-y-4">
      @csrf
      @method('PUT')
      ...
      <div>
        ...
        <input type="text" name="timezone" value="{{ old('timezone', $account->timezone ?? 'Asia/Manila') }}"
               class="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm">
      </div>

      <div class="pt-2 flex justify-between items-center">
        <form action="{{ route('bank-accounts.destroy', $account) }}" method="POST"
              onsubmit="return confirm('Delete this bank account? This is only allowed if there are no transactions/loans.');">
          @csrf
          @method('DELETE')
          <button class="inline-flex items-center px-3 py-2 rounded-lg bg-rose-50 text-rose-700 text-xs font-semibold hover:bg-rose-100">
            Delete
          </button>
        </form>
