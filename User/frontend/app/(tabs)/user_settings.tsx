...
import { api, getCurrentApiBaseUrl, setApiBaseUrl, refreshApiUrl } from '../../src/config/api';
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        >
          ...
          <Text style={styles.settingText}>API Server URL</Text>
          <Ionicons name={showApiInput ? "chevron-up" : "chevron-down"} size={20} color="#9CA3AF" style={{ marginLeft: 'auto' }} />
        </TouchableOpacity>

        {showApiInput && (
          <View style={styles.apiInputContainer}>
            <Text style={styles.apiLabel}>Current: {apiUrl}</Text>
            <TextInput
            ...
            <View style={styles.apiButtonRow}>
              <TouchableOpacity
                style={[styles.apiSaveBtn, styles.apiBtnHalf]}
                onPress={async () => {
                  try {
                    setLoading(true);
                    const newUrl = await refreshApiUrl();
                    setApiUrl(newUrl);
                    Alert.alert('Success', `API URL auto-detected: ${newUrl}`);
                  } catch (error: any) {
                    Alert.alert('Error', error.message || 'Failed to auto-detect API URL');
                  } finally {
                    setLoading(false);
                  }
                }}
              >
                <Text style={styles.apiSaveText}>Auto-Detect</Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={[styles.apiSaveBtn, styles.apiBtnHalf]}
                onPress={async () => {
                  try {
                    ...
                    Alert.alert('Error', error.message || 'Failed to update API URL');
                  } finally {
                    setLoading(false);
                  }
                }}
