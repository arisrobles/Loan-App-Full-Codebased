    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        <View style={styles.section}>
          ...
          {(() => {
            // Collect all upcoming repayments from all active loans
            ...
            loans.forEach((loan) => {
              if (loan.repayments && Array.isArray(loan.repayments)) {
                loan.repayments.forEach((rep: any) => {
                  // amountDue is the EMI (monthly payment amount) for this specific repayment
                  ...
                  if (amountPaid < emiAmount) {
                    const dueDate = new Date(rep.dueDate);
                    // Only show payments due in the next 90 days
                    const daysUntilDue = Math.ceil((dueDate.getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24));
                    
                    if (daysUntilDue >= 0 && daysUntilDue <= 90) {
                      // Show remaining amount due for this payment (EMI - amount already paid)
                      const remainingAmount = emiAmount - amountPaid;
                      upcomingPayments.push({
                        loanReference: loan.reference,
                        dueDate: rep.dueDate,
                        amountDue: remainingAmount.toFixed(2), // Remaining EMI amount to pay
                        loanId: loan.id,
                      });
                    }
                  }
                    ...
                    return (
                      <View
                        ...
                        style={[
                          ...
                          isDueSoon && styles.upcomingPaymentCardDueSoon,
                        ]}
                      >
...
const styles = StyleSheet.create({
  ...
  branding: { fontSize: 22, fontWeight: "700", color: "orange", letterSpacing: 1 },
  breakdownToggle: {
    flexDirection: "row",
    justifyContent: "center",
    alignItems: "center",
