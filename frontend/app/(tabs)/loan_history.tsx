...
export default function LoanHistoryScreen() {
  // router is available but not currently used - keeping for future navigation
  ...
  const getReceiptImageUrl = (fileUrl: string) => {
    ...
    const baseURL = api.defaults.baseURL?.replace('/api/v1', '') || 'http://192.168.8.105:8080';
              ...
              >
                ...
                {canCancelLoan(loan.status) && (
                  <TouchableOpacity
                    ...
                    onPress={() => handleCancelLoan(loan.id, loan.reference)}
